...
async def _compare_pets(name1: str, name2: str) -> tuple[str, UniMessage | None]:
    if not name2:
...
async def _find_pet_matches(name1: str) -> tuple[str, UniMessage | None]:
...
async def get_nrc_eggs_groups(action: str, name1: str, name2: str = "") -> tuple[str, UniMessage | None]:
    ...
    """
    if action == "compare":
        return await _compare_pets(name1, name2)
    if action == "find_matches":
        return await _find_pet_matches(name1)
    return (
