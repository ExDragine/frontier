...
async def hydrate_recent_media_context(
    ...
    *,
    ...
    workspace_key: str,
    memory_dir: str | Path,
) -> tuple[list[bytes], list[dict[str, object]], bool]:
    ...
    writer. Images and workspace file references are returned directly so the
    current-message-only Agent input does not need to re-render chat history.
    """
    ...
    return images, list(attachments) if isinstance(attachments, list) else [], True
