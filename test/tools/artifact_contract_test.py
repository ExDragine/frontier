...
def test_subagent_artifact_tools_use_staged_artifact_handoff():
    repo_root = Path(__file__).resolve().parents[2]
    offenders = []

    for path in sorted((repo_root / "tools").glob("*.py")):
        module_name = path.stem
        if module_name in MAIN_ARTIFACT_MODULES or module_name.startswith("__"):
            continue

        tree = ast.parse(path.read_text(encoding="utf-8"))
        for node in tree.body:
            if isinstance(node, ast.AsyncFunctionDef) and _is_content_and_artifact_tool(node):
                if not _uses_stage_artifact_response(node):
