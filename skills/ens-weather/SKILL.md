...
2. For a follow-up, comparison, or evaluation of data already shown in chat, call `get_recent_conversation` first. Reuse the translated values in history instead of calling ENS again. Only fetch again when history has no usable result.
3. For video, call `get_recent_conversation` to recover the previous parameters, then call `ens_normal(no_video=False)`.
