...
各服务独立超时；单服务故障不使全部工具失效。只保留 `mcp_get_tools_async()` 一个异步加载入口，
读取工具属性不会触发网络访问，也不会创建临时事件循环。
每个服务仅保存配置、已发现工具和下次重试时间。成功结果缓存至重启；失败后至少间隔 60 秒，
在后续请求中重试，不运行后台轮询。注册器将本地工具与 MCP 工具分开存放，按需组合；
主 Agent 每轮读取最新工具快照，无需 revision 同步，也不因 MCP 恢复而重建文档子代理。
发现日志包含服务名、工具数量或错误类型／HTTP 状态，不输出远端异常中的 URL 和凭据。
